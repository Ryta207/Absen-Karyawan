...
function tambahKaryawan() {
    ...
    const gaji =
        Number(document.getElementById("gaji").value);

    if (nama === "" || gaji <= 0) {
        alert("Masukkan nama dan gaji dengan benar.");
        return;
    }
...
function tampilkanData() {
    ...
    karyawan.forEach((item, index) => {
        ...
        const gajiBersih =
            gajiKotor - Number(item.kasbon);

        let hariHTML = "";

        item.hari.forEach((nilai, hariIndex) => {

            hariHTML += `
                <td>
                    <select
                        onchange="ubahHari(${index}, ${hariIndex}, this.value)"
                    >
                        <option value="0"
                            ${nilai == 0 ? "selected" : ""}>
                            Libur
            ...
            <tr>
                ...
                <td>
                    <input
                        ...
                        type="number"
                        value="${item.kasbon}"
                        onchange="ubahKasbon(${index}, this.value)"
                    >
                </td>
...
function ubahHari(index, hariIndex, nilai) {
...
function ubahKasbon(index, nilai) {
...
function buatLaporan(item) {
    ...
    item.hari.forEach((nilai, indexHari) => {

        absensiHTML += `
            <tr>
                <td>${namaHari[indexHari]}</td>
                <td>${statusHari(nilai)}</td>
            </tr>
        `;
...
function cetakKaryawan(index) {
    ...
    const laporan = buatLaporan(item);
    ...
    if (!jendela) {
        ...
        alert(
            "Popup diblokir oleh browser. Izinkan popup untuk website ini."
        );
    ...
    jendela.document.write(laporan);
...
function cetakSemua() {
    ...
    jendela.document.write(laporanSemua);
...
function kirimWhatsApp(index) {
    ...
    item.hari.forEach((nilai, indexHari) => {
        ...
        absensi +=
            `${namaHari[indexHari]} : ${statusHari(nilai)}\n`;
